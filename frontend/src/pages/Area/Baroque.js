...
export default function Cubism() {
  ...
  async function sampleFunc() {
    let response = await fetch("/api/artwork/areaId=11");
  ...
  return (
    ...
    <AppbarLoggedIn />
      <Fab  className={classes.fab} component={Link} to={window.location.pathname.replace("/Baroque", "")}>
			  <ArrowBackIcon />
		  </ Fab>
      {/* Hero unit */}
          ...
          {data.map((card) => (
            <Grid item key={card} xs={12} sm={6} md={6}>
              <Card className={classes.card}>
                ...
                <CardActions>
                <Link to={"/areaHub/Baroque/artwork/" + card.artworkId}>
